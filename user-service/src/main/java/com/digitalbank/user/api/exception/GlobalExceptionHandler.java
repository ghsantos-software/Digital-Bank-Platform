...
public class GlobalExceptionHandler {
    ...
    // Catches DB unique constraint violations that slip past the service-layer check (race conditions)
    @ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<ErrorResponse> handleDataIntegrity(
            DataIntegrityViolationException ex, HttpServletRequest request) {

        String message = "Data conflict: a record with the same unique field already exists";
        if (ex.getMessage() != null) {
            if (ex.getMessage().contains("uk_users_email")) message = "Email already in use";
            else if (ex.getMessage().contains("uk_users_cpf")) message = "CPF already registered";
        }
